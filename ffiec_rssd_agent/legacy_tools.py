"""
Legacy MCP tool definitions for the FFIEC RSSD Lookup Agent.

This module preserves the previous `tools.py` implementation (before MCP tools were
removed from the default agent). Use it for example runs or experiments:

  from ffiec_rssd_agent.legacy_tools import create_ffiec_server, ALLOWED_TOOL_NAMES

The interactive CLI (`agent.py`) does not import this module unless you wire it in.
"""
...
def create_ffiec_server():
    """Create the in-process MCP server with all FFIEC tools (legacy definitions)."""
